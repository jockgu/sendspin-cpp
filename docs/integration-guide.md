...
| `required_lead_time_ms` | `uint16_t` | `0` | Minimum presentation lead requested from the server, advertised in `client/state`. A value of `0` leaves lead-time selection to the server. |
| `min_buffer_ms` | `uint16_t` | `0` | Minimum ongoing audio buffer requested from the server, advertised in `client/state`. For live sources, use this to absorb normal delivery jitter before the local render FIFO drains. A value of `0` leaves buffering selection to the server. |
